package com.echomu.androidlib.utils.immersion;
...
public class OSUtils {
    ...
    private static final Pattern PatternEmotionUI  =  Pattern.compile("EmotionUI_([\\d.]+)");

    private static final Pattern PatternFlyme  =  Pattern.compile("Flyme[^\\d]*([\\d.]+)[^\\d]*");

    private static final Pattern PatternColorOS  =  Pattern.compile("ColorOS([\\d.]+)");

    private static final Pattern PatternOther = Pattern.compile("([\\d.]+)[^\\d]*");

    private static final Pattern PatternAmigo  =   Pattern.compile("amigo([\\d.]+)[a-zA-Z]*");
    /**
     * ROM 类型
     */
        ...
        try {
            ...
            if (buildProperties.containsKey(KEY_MIUI_VERSION_NANE) || buildProperties.containsKey(KEY_MIUI_VERSION_CODE)) {
                ...
                if (buildProperties.containsKey(KEY_EMUI_VERSION)) {
                    ...
                    Matcher matcher = PatternEmotionUI.matcher(versionStr); // EmotionUI_3.0
                ...
                if (buildProperties.containsKey(KEY_DISPLAY_ID)) {
                    ...
                    Matcher matcher = PatternFlyme.matcher(versionStr); // Flyme OS 4.5.4.2U
                    if (!TextUtils.isEmpty(versionStr) && matcher.find()) {
                        try {
                            String version = matcher.group(1);
                            rom.setVersion(version);
                ...
                if (buildProperties.containsKey(KEY_COLOROS_ROM_VERSION)) {
                    ...
                    Matcher matcher = PatternColorOS.matcher(versionStr); // ColorOS2.1
                    if (!TextUtils.isEmpty(versionStr) && matcher.find()) {
                        try {
                            String version = matcher.group(1);
                            rom.setVersion(version);
                ...
                if (buildProperties.containsKey(KEY_EUI_VERSION)) {
                    ...
                    Matcher matcher = PatternOther.matcher(versionStr); // 5.9.023S
                    if (!TextUtils.isEmpty(versionStr) && matcher.find()) {
                        try {
                            String version = matcher.group(1);
                            rom.setVersion(version);
                ...
                if (buildProperties.containsKey(KEY_DISPLAY_ID)) {
                    String versionStr = buildProperties.getProperty(KEY_DISPLAY_ID);
                    Matcher matcher = PatternAmigo.matcher(versionStr); // "amigo3.5.1"
                    if (!TextUtils.isEmpty(versionStr) && matcher.find()) {
                        try {
                            String version = matcher.group(1);
                            rom.setVersion(version);
